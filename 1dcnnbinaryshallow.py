...
def get_model_stats(model: nn.Module, input_dim: int) -> Dict[str, float]:
    ...
    # Rough FLOPs estimate for this CNN
    l1 = input_dim
    l1_pool = max(l1 // 2, 1)
    l2 = l1_pool
    l2_pool = max(l2 // 2, 1)
...
def make_client_fn(resources: ExperimentResources) -> Callable[[Context], fl.client.Client]:
    def client_fn(context: Context):
        ...
        model = CNN1D(input_dim=resources.input_dim).to(DEVICE)
...
def evaluate_final_global_model(
    ...
    for client_data in resources.clients:
        model = CNN1D(resources.input_dim).to(DEVICE)
...
if __name__ == "__main__":
    ...
    input_dim = X.shape[1]
    model_stats = get_model_stats(CNN1D(input_dim), input_dim)
