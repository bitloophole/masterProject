...
DATA_PATH = Path(__file__).resolve().parent / "data" / "iot_dataset_undersampled_mapped1.csv"
...
def aggregate_parameters(
    ...
    for layer_values in zip(*(parameters for parameters, _ in client_parameters)):
        reference = layer_values[0]
        weighted_sum = np.zeros(reference.shape, dtype=np.float64)

        for layer, (_, num_examples) in zip(layer_values, client_parameters):
...
def run_local_fedavg(strategy: MetricsFedAvg) -> None:
    print(
        "Ray is not available for this Python/Windows environment. "
        "Running local in-process FedAvg backend.\n"
    )
