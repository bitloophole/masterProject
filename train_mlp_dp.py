...
def get_model_stats(model: nn.Module, input_dim: int) -> Dict[str, float]:
    ...
    # Approximate FLOPs for the actual architecture:
    # input_dim -> 64 -> 32 -> 1
    # rough linear layer FLOPs: 2 * in * out
    forward_flops = (
        (2 * input_dim * 64)
        + (2 * 64 * 32)
        + (2 * 32 * 1)
...
def train_with_dp(
    ...
    # A common practical choice is delta < 1/N
...
class FLClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        # Rebuild a fresh model every round before privatization
...
def save_global_model(parameters: Parameters, input_dim: int, epsilon: float, path: str = "."):
    if parameters is None:
    ...
    filename = f"{path}/mlp_global_model_eps_{epsilon}.pt"
...
def run_dp_experiment(
    ...
    print("\n" + "=" * 70)
    print(f"STARTING DP EXPERIMENT WITH PRIVACY BUDGET ε = {epsilon}")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    # Save CSV if needed
    summary_df.to_csv("dp_federated_results_summary.csv", index=False)
    print("\nSaved summary to: dp_federated_results_summary.csv")

    # Comparison plots across epsilons
    ...
    print(f"\nTOTAL WALL-CLOCK TIME FOR ALL DP EXPERIMENTS: {total_runtime:.2f} s")