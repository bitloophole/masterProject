...
# Differential Privacy budgets to test
...
def unwrap_model(model: nn.Module) -> nn.Module:
    """
    Opacus wraps models inside GradSampleModule.
    This helper returns the original model if wrapping exists.
    """
...
class CNN1D(nn.Module):
    def __init__(self, input_dim: int) -> None:
        ...
        conv_output_length = max(input_dim // 4, 1)

        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(48 * conv_output_length, 96),
            nn.LayerNorm(96),
            nn.ReLU(),
...
def make_model(input_dim: int) -> nn.Module:
    ...
    if errors:
        print("\nOpacus validator found issues; applying ModuleValidator.fix().")
...
def get_model_stats(model: nn.Module, input_dim: int) -> Dict[str, float]:
    ...
    # Rough FLOPs estimate for the CNN
    l1 = input_dim
    l1_pool = max(l1 // 2, 1)
    l2 = l1_pool
    l2_pool = max(l2 // 2, 1)
...
def train_with_dp(
    ...
    # Common practical choice: delta < 1 / N
    ...
    for _ in range(epochs):
        ...
        private_model.eval()
        with torch.no_grad():
            val_logits = private_model(X_val_tensor)
...
def test(model: nn.Module, X: np.ndarray, y: np.ndarray) -> Dict[str, float]:
    ...
    # Use 0.5 to match your original 1D CNN code.
    # Change to 0.7 if you want to match your DP-MLP threshold.
...
class FLClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        ...
        return [
            ...
            for _, value in base_model.state_dict().items()
        ]

    def set_parameters(self, parameters) -> None:
        base_model = unwrap_model(self.model)
        state_dict = dict(zip(base_model.state_dict().keys(), parameters))
    ...
    def fit(self, parameters, config):
        # Rebuild a fresh model every round before Opacus privatization.
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        if aggregated_metrics:
            row = {
                ...
                "target_epsilon": float(
                    aggregated_metrics.get("target_epsilon", self.experiment_epsilon)
                ),
...
def plot_loss_curves(round_logs: List[Dict[str, float]], epsilon: float) -> None:
    ...
    plt.ylabel("Loss")
    plt.title(f"Training and Validation Loss per Round: DP 1D CNN, ε={epsilon}")
...
def plot_metric_vs_epsilon(summary_df: pd.DataFrame, metric: str, ylabel: str) -> None:
    ...
    plt.ylabel(ylabel)
    plt.title(f"{ylabel} vs Privacy Budget: DP 1D CNN")
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.show()
...
def save_global_model(
    ...
    filename = f"dp_1dcnnsh_epsilon_{epsilon:.1f}.pth"
...
def run_dp_experiment(
    ...
    print("\n" + "=" * 70)
    print(f"STARTING DP 1D CNN EXPERIMENT WITH PRIVACY BUDGET ε = {epsilon}")
    print("=" * 70)

    set_seed(RANDOM_STATE)

    dp_config = DPConfig(target_epsilon=epsilon, max_grad_norm=MAX_GRAD_NORM)
    ...
    saved_model_path = save_global_model(
        strategy.final_parameters,
        resources.input_dim,
        epsilon,
    )

    print(f"Saved final global model for ε={epsilon:.1f} to: {saved_model_path}")
    ...
    last_train_loss = (
        float(strategy.round_logs[-1]["train_loss"]) if strategy.round_logs else 0.0
    )
    last_val_loss = (
        float(strategy.round_logs[-1]["val_loss"]) if strategy.round_logs else 0.0
    )
    last_epsilon_spent = (
        float(strategy.round_logs[-1]["epsilon_spent"]) if strategy.round_logs else 0.0
    )
    last_noise_multiplier = (
        float(strategy.round_logs[-1]["noise_multiplier"]) if strategy.round_logs else 0.0
    )
    ...
    print(
        f"Round Train Time(sum of max/client-round): "
        f"{result['round_train_time_sum_of_max_sec']:.4f} s"
    )
    ...
    print(f"Round-wise Results for ε={epsilon} - copy into thesis table:")
...
if __name__ == "__main__":
    ...
    print("============== FEDERATED DP 1D CNN TRAINING ==============")
    print(
    ...
    summary_df.to_csv("dp_1dcnn_federated_results_summary.csv", index=False)
    print("\nSaved summary to: dp_1dcnn_federated_results_summary.csv")
    ...
    print(f"\nTOTAL WALL-CLOCK TIME FOR ALL DP 1D CNN EXPERIMENTS: {total_runtime:.2f} s")