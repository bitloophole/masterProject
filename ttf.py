...
def load_data() -> Tuple[np.ndarray, np.ndarray]:
    ...
    #print("iot_dataset_after_undersampling_ttf.csv")
...
class TabTransformer(nn.Module):
    def __init__(
        self,
        input_dim: int,
        embed_dim: int = 32,   # was 32
        num_heads: int = 4,    # was 4
        num_layers: int = 2,   # keep 2 for TTF structure
        ff_dim: int = 64,      # was 128
        dropout: float = 0.2,
    ) -> None:
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(input_dim * embed_dim, 64),  # was 128
            nn.ReLU(),
            nn.Dropout(dropout),
...
def get_model_stats(model: nn.Module, input_dim: int) -> Dict[str, float]:
    ...
    # Rough FLOPs estimate for TabTransformer
    if isinstance(model, TabTransformer):
        ...
        # Approximate per-layer transformer cost
        # QKV + output projection + attention scores/value mixing + FFN
        per_layer_flops = (
            8 * L * d * d
            + 4 * L * L * d
            + 4 * L * d * 64
...
def make_client_fn(resources: ExperimentResources) -> Callable[[Context], fl.client.Client]:
    def client_fn(context: Context):
        ...
        model = TabTransformer(input_dim=resources.input_dim).to(DEVICE)
...
def evaluate_final_global_model(
    ...
    for client_data in resources.clients:
        model = TabTransformer(resources.input_dim).to(DEVICE)
...
if __name__ == "__main__":
    ...
    input_dim = X.shape[1]
    model_stats = get_model_stats(TabTransformer(input_dim), input_dim)
    ...
    if strategy.final_parameters is not None:
        ...
        final_model = TabTransformer(input_dim).to(DEVICE)
        final_state_dict = dict(zip(final_model.state_dict().keys(), final_ndarrays))
        ...
        torch.save(final_model.state_dict(), "ttf_final_global_model.pth")

        print("\nSaved final trained TTF model to:")
        print("ttf_final_global_model.pth")
    ...
    for row in strategy.round_logs:
        print(
            ...
            f"{row['round_total_train_time_sec']:.4f}\t"
            f"{row['communication_bytes']:.0f}\t"
            f"{format_bytes(row['communication_bytes'])}"
